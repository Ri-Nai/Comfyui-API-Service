...
def style_transfer(base64_encoded_input_image):
...
def combined_style_transfer(base64_encoded_input_image):
    # --- 2. Example: Combined Style Transfer (Single Image) --- 
    url_combined_style_transfer = f"{BASE_URL}/generate/combined_style_transfer"
    print(f"调用 Combined Style Transfer API: {url_combined_style_transfer}")
    combined_style_transfer_request_data = {
        "input_image": base64_encoded_input_image,
    ...
    try:
        response = requests.post(url_combined_style_transfer, json=combined_style_transfer_request_data)
        response.raise_for_status()
        result = response.json()
        if result.get("status") == "success" and result.get("images"):
            for img_data in result["images"]:
                try:
                    save_path = ImageHelper.get_output_path(img_data["filename"])
                    ImageHelper.decode_and_save_image(img_data["data"], save_path)
                    print(f"Combined Style Transfer 图片已保存为: {save_path}")
                except Exception as e:
                    print(f"保存 Combined Style Transfer 图片失败: {str(e)}")
        else:
            print(f"Combined Style Transfer API 返回逻辑错误: {result.get('message', 'No message')} - Detail: {result.get('detail', result)}")
    except requests.exceptions.HTTPError as http_err:
        print(f"Combined Style Transfer HTTP 请求失败: {http_err}")
    except requests.exceptions.RequestException as req_err:
        print(f"Combined Style Transfer 请求错误: {req_err}")
    except Exception as e:
        print(f"处理 Combined Style Transfer 响应时发生意外错误: {e}")
    print("\n---\n")


def tagger(base64_encoded_input_image):
    # --- 2. Example: Tagger ---
...
def style_transfer_with_image(base64_encoded_input_image):
    # --- 3. Example: Style Transfer with Image (IPAdapter) ---
...
def main():
    ...
    base64_encoded_input_image = ImageHelper.encode_image(input_image_path)
    
    # 执行各种风格迁移操作
    # style_transfer(base64_encoded_input_image)
    combined_style_transfer(base64_encoded_input_image)
    # 取消注释下面的行以启用 tagger
    # tagger(base64_encoded_input_image)
    # style_transfer_with_image(base64_encoded_input_image)
